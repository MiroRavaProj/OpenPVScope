...
from openpvscope.segmentation.extract import run_segmentation
...
def start_segmentation_job(store: ProjectStore, *, margin_factor: float = 0.15) -> None:
    global _thread
    with _lock:
    ...
    def work() -> None:
        console.begin_job("Segmentation", detail="Pairing & crop extraction")
        ...
        try:
            result = run_segmentation(
                store.root, margin_factor=margin_factor, progress=progress
            )
            mark_step(
                store,
                "segmentation",
                StepStatus.DONE,
                message=f"{result['count']} pairs",
            )
            with _lock:
                _state.update({"running": False, "result": result, "error": None})
            console.end_job(ok=True, message=f"Segmented {result['count']} panels")
