...
def estimate_global_affine_akaze(
    ...
    if gsd_m is not None and gsd_m > 0:
        tile = max(32, int(round(TILE_M / gsd_m * f)))
        stride = max(16, int(round(STRIDE_M / gsd_m * f)))
    else:
...
def register_thermal_to_rgb(
    ...
    *,
    ...
    g_max = max_displacement_px(gsd_work, GLOBAL_MAX_M)
    l_max = max_displacement_px(gsd_work, LOCAL_MAX_M)
    tile = meters_to_px(TILE_M, gsd_work)
    stride = max(8, meters_to_px(STRIDE_M, gsd_work, min_px=8))
    if stride >= tile:
        stride = max(8, tile // 2)
    ...
    M_work, pass1_meta = estimate_global_affine_akaze(
        ref, mov, max_shift_px=g_max, gsd_m=gsd_work
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Register thermal to RGB (affine Pass 1 + metre-tile TPS Pass 2); "
